...
## Treated with antivirals (Hepatitis E)

```{r fig.height=9, fig.width=9, dpi=144, echo=F, message=F, warning=F, results="asis"}

cat("\n\n")
cat(tab_caption("tav"," Hepatitis E patients who recevied antivirals ."))
cat("\n\n")

baza_de_date %>%
  filter(`Tratament antiviral`=="yes") %>%
  mutate(`Data internarii`  = `Data internarii` %>% as.numeric %>% as.Date(origin = "1899-12-30")) %>%
  mutate(`Data externarii`  = `Data externarii` %>% as.numeric %>% as.Date(origin = "1899-12-30")) %>%
  flextable::flextable() %>%
  flextable::autofit()

```

## Treated with plasma (Hepatits A)

```{r fig.height=9, fig.width=9, dpi=144, echo=F, message=F, warning=F, results="asis"}

cat("\n\n")
cat(tab_caption("tplasma","Hepatits A patients who recevied plasma."))
cat("\n\n")

baza_de_date %>%
  filter(`Group`=="A") %>%
  filter(`Bad outcome`=="yes") %>% 
  mutate(`Data internarii`  = `Data internarii` %>% as.numeric %>% as.Date(origin = "1899-12-30")) %>%
  mutate(`Data externarii`  = `Data externarii` %>% as.numeric %>% as.Date(origin = "1899-12-30")) %>%
  flextable::flextable() %>%
  flextable::autofit()
...
cat("\n\n")
cat(tab_caption("tdied","Patients who died."))
cat("\n\n")
